...
def mode_select(num):
    mode = [ '🖐️   About App' , '💉   가상 성형 AI' , '👧   피부 진단 AI', '🏥   입점 병원', '⏰   병원 상세 & 예약',
           '🌈   메인페이지 UI 샘플',
           '🏠   병원 상세 UI 샘플(1)', '🏠   병원 상세 UI 샘플(2)', '🏠   병원 상세 UI 샘플(3)']
...
if app_mode == '🖐️   About App':
    ...
    if st.button("결과 보기!" , key='res'):
        ...
        col3.image(sequence_list[index])
        col4.image(remove(sequence_list[index],post_process_mask=True,bgcolor=(255, 255, 255, 255)))
